...
def preprocess_images_outpainting(imgs, crop=True, position='middle'):
    ...
    if crop:
        if position == 'middle':
            ...
            '''
             给每一张图片的左右两边32像素宽的地方重新赋值为这幅图的均值
            '''
            mask = np.zeros((m, IMAGE_SZ, IMAGE_SZ, 1))
            mask[:, :, :int(2 * IMAGE_SZ / 8), :] = mask[:, :, int(-2 * IMAGE_SZ / 8):, :] = 1.0
        elif position == 'left':
            imgs[:, :, :int(2 * IMAGE_SZ / 8), :] = pix_avg[:, np.newaxis, np.newaxis, np.newaxis]
            mask = np.zeros((m, IMAGE_SZ, IMAGE_SZ, 1))
            mask[:, :, :int(2 * IMAGE_SZ / 8), :] = 1.0
        elif position == 'right':
            imgs[:, :, int(-2 * IMAGE_SZ / 8):, :] = pix_avg[:, np.newaxis, np.newaxis, np.newaxis]
            mask = np.zeros((m, IMAGE_SZ, IMAGE_SZ, 1))
            mask[:, :, int(-2 * IMAGE_SZ / 8):, :] = 1.0
    ...
    for i in range(m):
        ...
        #print(imgs[i].shape)
        #print(mask[i].shape)
        imgs_p.append(np.concatenate((imgs[i], mask[i]), axis=2))
...
def sample_random_minibatch(data, data_p, m):
    ...
    return np.asarray(data)[indices], np.asarray(data_p)[indices]
...
def resize_images(src_PATH, dst_PATH):
    for filename in os.listdir(src_PATH):
        ...
        img_scale = img_crop.resize((IMAGE_SZ, IMAGE_SZ), Image.ANTIALIAS) # resize操作采用平滑滤波
        full_outfilename = os.path.join(os.path.abspath(dst_PATH), os.path.splitext(filename)[0] + '.png')
